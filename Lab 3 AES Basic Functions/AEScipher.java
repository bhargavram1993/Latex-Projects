...
public class AEScipher {
  ...
  /**
   * Converting the input string message and keys into a matrix so that it
   * becomes easier both visually and for calculation.
   * 
   * @returns a tempMatrix which stores the input message, input key depending
    ...
    try {
      ...
      for (int kMatrixColumn = 0; kMatrixColumn < 4; kMatrixColumn++) {
        for (int kMatrixRow = 0; kMatrixRow < 4; kMatrixRow = kMatrixRow + 1) {
          tempMatrix[kMatrixRow][kMatrixColumn] = key
              .substring(keyTraversalCounter, keyTraversalCounter + 2);
      ...
      System.err.println("Error occured in inputtingStringIntoKMatrix mathod, "
          + "generation of input 4x4 matrix not possible ");
    }
    return tempMatrix;
  }
  ...
  public static void copyingIntoWHexMatrix() {
    ...
    try {
      ...
      System.err.println(
          "Error occured in copyingIntoWHexMatrix() method, couldn't "
          + "copy values from 4x4 martix to 4x44 matrix ");
    }
  }
   ...
   * keys from sBoxTable are fetched and split.
   * 
   * Parameters: sBoxTableInput:This parameter takes input from sBoxTable
   */
  public static String sBoxCipher(String sBoxTableInput) {
    try {
      String keys = sBoxTable[Integer.parseInt(sBoxTableInput.split("")[0],
          16)][Integer.parseInt(sBoxTableInput.split("")[1], 16)];
      // System.out.println(Keys);
      return keys;
    } catch (Exception sBox) {
      System.err.println(
          "Error occured in sBoxCipher method, Could'nt return keys from sBox ");
      return "";
    }
  }
  ...
  public static String rConCipher(int rConTableInput) {
    try {
      ...
      System.err.println(
          "Error occured in rConCipher method, Couldn't return keys from rCon");
      return "";
    }
  }
   ...
   * column wise starting from the 5th column, by fetching inputs from the user
   * and calculates round keys using sBoxTable and rConTable. The first 4
   * columns are filled, for the rest 40 columns we do the following operations.
   * 
   */
    ...
    try {
      ...
      for (int finalMatrixColumn = 4; finalMatrixColumn < 44; finalMatrixColumn++) {
        if (finalMatrixColumn % 4 != 0) {
          ...
          temporaryMatrix[0][0] = xorCaliculation(rConCipher(r),
              temporaryMatrix[0][0]);
          for (int fianlMatrixRow = 0; fianlMatrixRow < 4; fianlMatrixRow++) {
            ...
            finalMatrix[fianlMatrixRow][finalMatrixColumn] = xorCaliculation(
                finalMatrix[fianlMatrixRow][finalMatrixColumn - 4],
      ...
      System.err.println(
          "Error occured in finalKeyMatrix() method, Couldn't perform "
          + "left shift and XOR operations ");
    }
  }
   ...
   * roundKeyMatrix
   *
   * This function generates the round keys
   * 
   */
  public static void roundKeyMatrix() {
    try {
      StringBuilder val = new StringBuilder();
      int printCounter = 0;
      int b = 0;
      // 11 round keys are generated.
      ...
      System.err.println("Error in generating round keys ");
    }
  }
   ...
   * xorCaliculation
   *
   * This function defines a xor calculation and this xorCaliculation method is
   * used by finalKeyMatrix and aesAddKey method.
   * 
   ...
   * aesAddKey
   *
   * inputs and output are four by four matrices where every element is a pair
   * of hex digits and that will perform the Add Round Key operation; that is,
   * the entries of the output matrix are simply the XOR of the corresponding
    ...
    try {
      ...
      for (int i = 0; i < keyMatrix.length; i++) {
        for (int j = 0; j < keyMatrix[i].length; j++) {
          aesXORMatrix[i][j] = xorCaliculation(keyMatrix[i][j],
              messageMatrix[i][j]);
        }
      }
   ...
   * the AES S-Box.
   * 
   * Returns the nibbledSubstitutionMatrix result.
   * 
   */
  ...
  public static String[][] aesNibbleSubstitution(String[][] Key) {
    ...
    } catch (Exception nibblesubstitution) {
      return null;
    }
  }
   ...
   * aesShiftRows
   *
   * inputs and output are 4 by 4 matrices of pairs of hex digits ant will
   * perform the Shift Row operation of the AES to transform the input state
  ...
  public static String[][] aesMixColumn(String[][] Key) {
    try {
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          aesMixColumnMatrix[i][j] = mixColumnAddition(Key, mixColumnMatrix, i,
              j);
        }
      }
   ...
   * 
   */
  public static String mixColumnAddition(String[][] Key, int[][] mixclmn, int i,
      int j) {
    int sum = 0;
    for (int k = 0; k < 4; k++) {
      int a = mixclmn[i][k];
   ...
   * simply usees pre-calculated lookup tables to perform the byte
   * multiplication by 2
   */

  public static int[][] twoMultiplicationMatrix   = {
   ...
   * by 3
   */
  public static int[][] threeMultiplicationMatrix = {
  ...
  public static int mixColumnMultiplication(int a, int b) {
    if (a == 1) {
      ...
      return twoMultiplicationMatrix[b / 16][b % 16];
    } else if (a == 3) {
      return threeMultiplicationMatrix[b / 16][b % 16];
    }
    return 0;
  }
}