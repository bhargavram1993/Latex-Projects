...
public class AesCipher {
  ...
  static int roundKeyMatrixColumn;
  static int round;
   ...
   */
  public static void matrixInformation(){
    if(columnValue==4){
    ...
    keyMatrix= new String[4][columnValue];
  }
  ...
  public static void computingMatrix() {
    ...
    System.out.println(roundKeyMatrixColumn);
    try {
      ...
      for (int column = columnValue; column < roundKeyMatrixColumn; column++) {
        // If the Column index is not a multiple of 4.
        // We XOR as denoted in the following xorCaliculation:
        if (column % columnValue != 0) {
          if(column % 4 ==0 && columnValue ==8){
            for (int row = 0; row < 4; row++) {
              ...
              roundKeyMatrix[row][column] = xorComputing( roundKeyMatrix[row][column],
                  roundKeyMatrix[row][column-columnValue]);
            }
          }else
            for (int row = 0; row < 4; row++) {
          ...
          for (int row = 0; row < 4; row++) {
            roundKeyMatrix[row][column] = xorComputing(
                roundKeyMatrix[row][column - columnValue], wNewMatrix[0][row]);
          }
        }
      }
      printKeys();
  ...
  private static String aesRcon(int round) {
    int rowNumber = 0;
    return R_CON[rowNumber][round / columnValue];
  }

  /**
   * Printing generated round keys
   */
  private static void printKeys() {
    ...
    while (count < numberOfRounds) {
      for (int column = 0; column < 4; b++, column++) {
        for (int a = 0; a < 4; a++) {
          System.out.print(roundKeyMatrix[a][b].toUpperCase());
        }
      }
      System.out.println("");
      count++;
   ...
   */
  static String[][] constantmatrix = { { "02", "03", "01", "01" },
   ...
   * 
   * @param value: Taking input         
   * @return returns xorComputed output
   */
  public static String multiplication(String valueIn) {
  ...
  static String mixoutput[][] = new String[4][4];
   ...
   */
  static int  loop;
  public static  void aes(String text, String key) {
    int inputLength = key.length()/8;
    columnValue = inputLength;
    matrixInformation();
    aesRoundKeys(key);
    matrixCreation(text);
