...
public class Decryption {
  ...
  static int roundKeyMatrixColumn = 0;
  static int round;
  ...
  public static void aes(String text, String key) {
    ...
    int count=0;
    for (int wmatCol = roundKeyMatrix[0].length - 4; wmatCol >= 0 && count <= round-1; wmatCol -= 8) {
      ...
      System.out.println("Came here : " + wmatCol + " round : " + round);
    }
  }
   ...
   */
  public static String[][] inverseAesNibbleSubstitution(String[][] inStateHex) {

    for (int row = 0; row < 4; row++) {
      for (int column = 0; column < 4; column++) {
        inStateHex[row][column] = invaesSBox(inStateHex[row][column]);
      }
    }
    return inStateHex;
  }
   ...
   */
  public static final int[][] fourteenMultiplicationMarix = {
